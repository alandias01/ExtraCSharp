...
namespace Pledger.Data
{
    public class PledgeProcessor
    {
        ...
        {
            /*
            The reason for all the if stmts
            If we aren't inserting to DTC and Apex, then we won't insert into blob.slate.pledge
            In this case, we will have issues with internal unique id, so we set it
            For OCC, we can set it to 199390, must be at least 6 digits
            ...
            */
            ...
            int UniqueIdForEID = SlatePledgeItem.PledgeId;
            int internalid = SlatePledgeItem.PledgeId;
            ...
            if (!sendToDTC || !sendToG1)
            {
                SlatePledgeItem.PledgeId = 199390;
            }

            int ReturnedDTCID = 0;
            if (sendToDTC)
            {
                ReturnedDTCID = PledgeStockToOCC(SlatePledgeItem.PledgeId, stp.RealTimePosition.cusip, stp.QuantityToPledge, stp.RealTimePosition.ParticipantNum);
            }
            ...
            if (sendToG1)
            {
                ...
                {
                    internalid = -1;
                    SlatePledgeItem.PledgeId = uniqueid;
                }

                string EID = "plg_" + SlatePledgeItem.PledgeId.ToString() + "_" + DateTime.Today.ToString("yyyyMMdd");
                ReturnedG1ID =
        ...
        public int UnpledgeStockFromOCC(int PledgeIdFromLocalTable, string CUSIP, int SharesToUnpledge, string clearingNo)
        {
        ...
        public int PledgeStockToOCC(int PledgeIdFromLocalTable, string CUSIP, int SharesToPledge, string clearingNo)
        {
