...
namespace Pledger.View
{
    public class ManualPledgeViewModel : NPC, IPledgeViewModel
    {
        ...
        public AsyncObservableCollection<spAlanGetRealTimePositions_Pledger_Result> RealTimePos { get; set; }
        ...
        {
            var UnpledgeListFromRealTimePos = RealTimePos.Where(x => x.SharesYouWantToUnpledge > 0).ToList();
            var PledgeListFromRealTimePos = RealTimePos.Where(x => x.SharesYouWantToPledge > 0).ToList();

            
            //Get Cusips w/o a price
            List<string> UnpledgeListStockswoPriceArr = UnpledgeListFromRealTimePos.Where(x => x.Price == null).Select(x => x.cusip).ToList();
            List<string> PledgeListStockswoPriceArr = PledgeListFromRealTimePos.Where(x => x.Price == null).Select(x => x.cusip).ToList();
            
            
            List<string> PledgeUnpledgeListStockswoPriceArr = UnpledgeListStockswoPriceArr.Concat(PledgeListStockswoPriceArr).ToList();

            if (PledgeUnpledgeListStockswoPriceArr.Count>0)
            {
                string StockswoPriceString = string.Join(System.Environment.NewLine, PledgeUnpledgeListStockswoPriceArr);
            ...
            UnpledgeListFromRealTimePos.ForEach(x => StocksToUnpledge.Add(new OCCStockToUnpledge(x, x.SharesYouWantToUnpledge)));
            PledgeListFromRealTimePos.ForEach(x => StocksToPledge.Add(new OCCStockPledge(x, x.SharesYouWantToPledge)));
